...
def compare_days(stock, close_prices, RSI):
    ...
    while Days_Observed < len(close_prices) - 5:
        if RSI[Days_Observed] <= 30:
            if (
                (
                    close_prices[Days_Observed + 1]
                    + close_prices[Days_Observed + 2]
                    + close_prices[Days_Observed + 3]
                    + close_prices[Days_Observed + 4]
                    + close_prices[Days_Observed + 5]
                )
                / 5
            ) > close_prices[Days_Observed]:
                True_Positive += 1
            else:
                False_Negative += 1
            Crosses += 1
...
def run(stock, start_date, rsi_comparison, current=False):
    ...
    if current:
        try:
            df = pd.read_csv(
                os.path.join(
                    os.getcwd(),
                    f"datasets\\scraped\\{stock}",
                    f"{stock}-total-data.csv",
                )
            )
            close_prices = df["close"].values
        except FileNotFoundError:
            df = yf.download(stock, start_date, today)
            close_prices = df["Close"].values
            print(df.tail(1))
    else:
        df = yf.download(stock, start_date, today)
    ...
    rsi_comparison = rsi_comparison.append(
...
if __name__ == "__main__":
    ...
    for ticker in symbols:
        rsi_comparison, avg_gain, avg_loss = run(
            stock=ticker, start_date=start_date, rsi_comparison=rsi_comparison
        )
        print("---------------------------------------------")
